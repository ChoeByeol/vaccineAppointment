...
public class StockPagination {
	...
	//필수 데이터
		...
		public StockPagination(HttpServletRequest req) {
			try {
		...
		private List<Stock3Vo> list;
		public void calculate() throws Exception {
			//count 계산
			StockDao stockDao = new StockDao();
			if(isSearch()) {
				this.count = stockDao.count(column, keyword);
			}
			else {
				this.count = stockDao.count();
			}
			...
			//list 계산
			if(this.isSearch()) {
				this.list = stockDao.searchByRownum(column, keyword, begin, end);
			}
			else {
				this.list = stockDao.listByRownum(begin, end);
			}
		}
		public int getP() {
			return p;
