...
public class ShotPagination {
	...
	//필수 데이터
		...
		public ShotPagination(HttpServletRequest req) {
			try {
		...
		private List<ShotDto> list;
		public void calculate() throws Exception {
			//count 계산
			ShotDao shotDao = new ShotDao();
			if(isSearch()) {
				this.count = shotDao.count(column, keyword);
			}
			else {
				this.count = shotDao.count();
			}
			...
			//list 계산
			if(this.isSearch()) {
				this.list = shotDao.searchByRownum(column, keyword, begin, end);
			}
			else {
				this.list = shotDao.listByRownum(begin, end);
			}
		}
		public int getP() {
			return p;
